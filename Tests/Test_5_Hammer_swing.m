...
u_max = 45;
%% File sbr_xy_hammer.ttt needs to be opened
clc;
time = 0:0.004:3;
x = [0 0*pi/180 0 0 0];
...
%x_data_pid = zeros(length(time), 3);
...
%x_data_pid(x_data_pid==0) = NaN;
%u_data_pid(u_data_pid==0) = NaN;
...
fprintf(txt + "\n");
max(abs(x_data_pid(:,2)))
max(abs(x_data_smc(:,2)))
max(abs(x_data_sw(:,2)))
