...
r_tha(:,find(time == 0.5):find(time == 2.5)) = 1;
...
hold on; plot(time, r_tha(1,:), '-', 'Color', '#7E2F8E'); hold off;
xlabel('Time (sec)'); ylabel('Linear velocity (m/s)'); legend('PID', 'SMC', 'Switch', 'Ref');
xlim([0 time(end)]);
