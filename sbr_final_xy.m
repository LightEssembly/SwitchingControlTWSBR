function [x_data, u_data, sigma_data] = sbr_final_xy(time, x_initial, K_val, Kp1_val, Ki1_val, Kd1_val, N1_val, N_bar, smc1_val, epsil_val, N_bar_smc1, r_tha_val, K2_val, Kp2_val, Ki2_val, Kd2_val, N2_val, N_bar2, r_sig_val, u_max, switchAngle)
    ...
    if (clientID>-1)
        ...
        for i=1:length(time)
            ...
            if (code == sim.simx_return_ok && i > 1)
                ...
                x(1:3) = fVal(1:3);
                sig(1:2) = fVal(5:6);
                if (x_data(i-1,4) - sig(1)) >  350*pi/180
                    sig(1) = floor(x_data(i-1, 4)/pi)*pi + abs(sig);
                end
                ...
                if abs(x(2)) < switchAngle
                    % PID is active
                    r = r_tha_val(1:2,i)*N_bar/2.25;
                    [~] = ode45(@pidFunctionTha, double([time(i) time(i)+dt]), x(1:3));
                    u_combined = u + u2;
                else
                    % SMC is active
                    r = r_tha_val(1:3,i)*N_bar_smc1;
                    [~, yOut] = ode45(@odeTha, double([time(i) time(i)+dt]), x(1:4));
                    xd = yOut(end, 1); theta = yOut(end, 2); thetad = yOut(end, 3); z = yOut(end, 4);
                    x(4) = z;
                    MF = invM_F(xd, theta, thetad);
                    cmu = epsilone*z - smc1*MF;
                    u_smc = cmu/(smc1*invM(xd, theta, thetad)*B_smc);
                    u_smc = [u_smc/2; u_smc/2];
                    u_combined = u_smc + u2;
                end
...
function dxdt = pidFunctionTha(t, x)
    global A B_pid K Kp1 Ki1 Kd1 N1 r u
    persistent t_prev ep_old ep ed ei ed_integ
    dxdt = zeros(3,1);
